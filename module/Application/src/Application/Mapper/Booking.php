...
{   
    ...
    public function fetchBookingsById ($id)
    {
    	...
    	$statement = $this->adapter->createStatement();
    	$statement->prepare("SELECT b.bookingid AS b_bookingid, b.name AS b_name, b.description AS b_description, b.participant_description AS b_participant_description, b.start AS b_start, b.end AS b_end, b.isprebooking AS b_isprebooking, b.isdeleted AS b_isdeleted, u_r.userid AS u_r_userid, u_r.roleid AS u_r_roleid, u_r.ldapid AS u_r_ldapid, u_r.loginname AS u_r_loginname, u_r.firstname AS u_r_firstname, u_r.lastname AS u_r_lastname, u_r.emailaddress AS u_r_emailaddress, u_b.userid AS u_b_userid, u_b.ldapid AS u_b_ldapid, u_b.loginname AS u_b_loginname, u_b.firstname AS u_b_firstname, u_b.lastname AS u_b_lastname, u_b.emailaddress AS u_b_emailaddress, r.resourceid AS r_resourceid, r.isbookable AS r_isbookable, r.isdeleted AS r_isdeleted, r.name AS r_name, r.description AS r_description, r.color AS r_color, u_b.roleid AS u_b_roleid, e.equipmentid AS e_equipmentid, p.size AS p_size, p.seatnumber AS p_seatnumber, p.placeid AS p_placeid, c.containmentid AS c_containmentid, c.parent AS c_parent, c.name AS c_name, c.description AS c_description, h.hirachyid AS h_hirachyid FROM Users u_b RIGHT OUTER JOIN Bookings b ON u_b.userid = b.booking_userid LEFT OUTER JOIN Users u_r ON b.responsible_userid = u_r.userid LEFT OUTER JOIN Resources r ON b.resourceid = r.resourceid LEFT OUTER JOIN Equipments e ON r.resourceid = e.resourceid LEFT OUTER JOIN Places p ON r.resourceid = p.resourceid LEFT OUTER JOIN Containments c ON r.resourceid = c.child LEFT OUTER JOIN Hirachies h ON c.hirachyid = h.hirachyid WHERE b.bookingid = " . $id . ";");
